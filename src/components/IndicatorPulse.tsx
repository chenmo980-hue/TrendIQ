...
export const IndicatorPulse: React.FC<IndicatorPulseProps> = ({ judgment }) => {
  if (!judgment || !judgment.indicatorsSummary) {
    return null;
  }
  ...
  const getDotColor = (status?: 'bullish' | 'bearish' | 'neutral') => {
    if (status === 'bullish') return 'bg-rose-500 shadow-[0_0_6px_rgba(244,63,94,0.6)]';
    if (status === 'bearish') return 'bg-emerald-400 shadow-[0_0_6px_rgba(52,211,153,0.6)]';
    return 'bg-slate-400';
  };
  ...
  return (
    <div className="bg-[#0e1319] border border-[#1d2631] rounded-lg px-4 py-2.5 grid grid-cols-2 sm:grid-cols-5 gap-3">
      {list.map(({ label, item }) => {
