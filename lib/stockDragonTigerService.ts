...
export async function fetchStockDragonTigerDetail(code: string): Promise<StockDragonTigerDetail> {
  ...
  try {
    ...
    const [bResp, sResp] = await Promise.all([
      fetch(buyUrl, { signal: controller.signal, headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.0.0 Safari/537.36', Connection: 'close' } }),
      fetch(sellUrl, { signal: controller.signal, headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.0.0 Safari/537.36', Connection: 'close' } }),
    ]);
    clearTimeout(timer);
